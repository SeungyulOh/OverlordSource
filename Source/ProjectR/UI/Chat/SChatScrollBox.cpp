...
SChatScrollBox::FSlot& SChatScrollBox::Slot()
{
	return *(new SChatScrollBox::FSlot());
}
...
public:
	...
	void Construct(const FArguments& InArgs, const TArray<SChatScrollBox::FSlot*>& InSlots)
	{
		PhysicalOffset = 0;
...
public:
	...
	virtual void OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const override
	{
		...
		{
			const SChatScrollBox::FSlot& ThisSlot = Children[SlotIndex];
	...
	TPanelChildren<SChatScrollBox::FSlot> Children;

private:

	float ArrangeChildVerticalAndReturnOffset(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren, const SChatScrollBox::FSlot& ThisSlot, float CurChildOffset) const
	{
	...
	float ArrangeChildHorizontalAndReturnOffset(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren, const SChatScrollBox::FSlot& ThisSlot, float CurChildOffset) const
	{
		const FMargin& ThisPadding = ThisSlot.SlotPadding.Get();
		const FVector2D& WidgetDesiredSize = ThisSlot.GetWidget()->GetDesiredSize();
...
void SChatScrollBox::Construct(const FArguments& InArgs)
{
	check(InArgs._Style);
	...
	if (InArgs._ExternalScrollbar.IsValid())
	{
		...
		ScrollBar->SetOnUserScrolled(FOnUserScrolled::CreateSP(this, &SChatScrollBox::ScrollBar_OnUserScrolled));
...
TSharedPtr<SScrollBar> SChatScrollBox::ConstructScrollBar()
{
	return TSharedPtr<SScrollBar>(SNew(SScrollBar)
		.Style(ScrollBarStyle)
		.Orientation(Orientation)
		.OnUserScrolled(this, &SChatScrollBox::ScrollBar_OnUserScrolled));
}

void SChatScrollBox::ConstructVerticalLayout()
{
	...
	+ SOverlay::Slot()
		...
		.ColorAndOpacity(this, &SChatScrollBox::GetStartShadowOpacity)
	...
	+ SOverlay::Slot()
		...
		[
			...
			SNew(SImage)
			.Visibility(EVisibility::HitTestInvisible)
		.ColorAndOpacity(this, &SChatScrollBox::GetEndShadowOpacity)
...
void SChatScrollBox::ConstructHorizontalLayout()
{
...
SChatScrollBox::FSlot& SChatScrollBox::AddSlot()
{
	SChatScrollBox::FSlot& NewSlot = *new SChatScrollBox::FSlot();
...
void SChatScrollBox::RemoveSlot(const TSharedRef<SWidget>& WidgetToRemove)
{
	TPanelChildren<SChatScrollBox::FSlot>& Children = ScrollPanel->Children;
	for (int32 SlotIndex=0; SlotIndex < Children.Num(); ++SlotIndex)
	{
...
void SChatScrollBox::ClearChildren()
{
	ScrollPanel->Children.Empty();
}

bool SChatScrollBox::IsRightClickScrolling() const
{
...
float SChatScrollBox::GetScrollOffset()
{
	return DesiredScrollOffset;
}

void SChatScrollBox::SetScrollOffset(float NewScrollOffset)
{
	DesiredScrollOffset = NewScrollOffset;
	bScrollToEnd = false;
}

void SChatScrollBox::ScrollToStart()
{
	SetScrollOffset(0);
}

void SChatScrollBox::ScrollToEnd()
{
	bScrollToEnd = true;
...
float SChatScrollBox::GetScrollOffsetEnd()
{
	...
	return ContentSize - GetScrollComponentFromVector(ScrollPanelGeometry.Size);
}

void SChatScrollBox::ScrollDescendantIntoView(const TSharedPtr<SWidget>& WidgetToFind, bool InAnimateScroll, EDescendantScrollDestination InDestination)
{
	...
	RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateSP(this, &SChatScrollBox::UpdateInertialScroll));

	Invalidate(EInvalidateWidget::Layout);
}

bool SChatScrollBox::ScrollDescendantIntoView(const FGeometry& MyGeometry, const TSharedPtr<SWidget>& WidgetToFind, bool InAnimateScroll, EDescendantScrollDestination InDestination)
{
	// We need to safely find the one WidgetToFind among our descendants.
	TSet< TSharedRef<SWidget> > WidgetsToFind;
	{
		WidgetsToFind.Emplace(WidgetToFind.ToSharedRef());
	}
...
EOrientation SChatScrollBox::GetOrientation()
{
	return Orientation;
}

void SChatScrollBox::SetOrientation(EOrientation InOrientation)
{
...
void SChatScrollBox::SetScrollBarVisibility(EVisibility InVisibility)
{
	ScrollBar->SetUserVisibility(InVisibility);
}

void SChatScrollBox::SetScrollBarAlwaysVisible(bool InAlwaysVisible)
{
	ScrollBar->SetScrollBarAlwaysVisible(InAlwaysVisible);
}

void SChatScrollBox::SetScrollBarThickness(FVector2D InThickness)
{
	ScrollBar->SetThickness(InThickness);
}

EActiveTimerReturnType SChatScrollBox::UpdateInertialScroll(double InCurrentTime, float InDeltaTime)
{
...
void SChatScrollBox::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
...
bool SChatScrollBox::ComputeVolatility() const
{
	return bIsScrolling || IsRightClickScrolling();
}

FReply SChatScrollBox::OnPreviewMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SChatScrollBox::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SChatScrollBox::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SChatScrollBox::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void SChatScrollBox::OnMouseLeave(const FPointerEvent& MouseEvent)
{
...
FReply SChatScrollBox::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
bool SChatScrollBox::ScrollBy(const FGeometry& AllottedGeometry, float ScrollAmount, EAllowOverscroll Overscrolling, bool InAnimateScroll)
{
...
FReply SChatScrollBox::OnDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Handled().CaptureMouse(SharedThis(this));
}

FCursorReply SChatScrollBox::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	if (IsRightClickScrolling())
	{
...
FReply SChatScrollBox::OnTouchStarted(const FGeometry& MyGeometry, const FPointerEvent& InTouchEvent)
{
...
FReply SChatScrollBox::OnTouchMoved(const FGeometry& MyGeometry, const FPointerEvent& InTouchEvent)
{
...
FReply SChatScrollBox::OnTouchEnded(const FGeometry& MyGeometry, const FPointerEvent& InTouchEvent)
{
	CachedGeometry = MyGeometry;
...
FNavigationReply SChatScrollBox::OnNavigation(const FGeometry& MyGeometry, const FNavigationEvent& InNavigationEvent)
{
...
int32 SChatScrollBox::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
void SChatScrollBox::ScrollBar_OnUserScrolled(float InScrollOffsetFraction)
{
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
...
FSlateColor SChatScrollBox::GetStartShadowOpacity() const
{
...
FSlateColor SChatScrollBox::GetEndShadowOpacity() const
{
...
bool SChatScrollBox::CanUseInertialScroll(float ScrollAmount) const
{

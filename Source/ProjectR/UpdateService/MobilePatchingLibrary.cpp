...
float UMobileInstalledContent::GetFreeDiskSpace()
{
...
bool UMobileInstalledContent::Mount(int32 InPakOrder, const FString& InMountPoint)
{
	...
	for (const FString& FileName : InstalledFileNames)
	{
		if (FPaths::GetExtension(FileName) == TEXT("pak"))
		{
			...
			{
				UE_LOG(LogMobilePatchingUtils, Log, TEXT("Failed to mount pak = %s"), *PakFullName);
				bMounted = false;
				break;
			}
		}
...
static void OnInstallComplete(bool bSuccess, IBuildManifestRef RemoteManifest, UMobilePendingContent* MobilePendingContent, FOnContentInstallSucceeded OnSucceeded, FOnContentInstallFailed OnFailed)
{
	if (MobilePendingContent == nullptr)
	{
		...
		UE_LOG(LogMobilePatchingUtils, Log, TEXT("Installation Failed. MobilePendingContent is null"));
		return;
	}
	...
	if (bSuccess)
	{
		...
		{
			UE_LOG(LogMobilePatchingUtils, Log, TEXT("Failed to save manifest to disk. %s"), *ManifestFullFilename);
		}
	...
	else
	{
		...
		UE_LOG(LogMobilePatchingUtils, Log, TEXT("Installation Failed. Code %d Msg: %s"), (int32)ErrorCode, *ErrorText.ToString());
...
void UMobilePendingContent::StartInstall(FOnContentInstallSucceeded OnSucceeded, FOnContentInstallFailed OnFailed)
{
	...
	float DiskFreeSpace = GetFreeDiskSpace();
	...
	Installer = BuildPatchServices->StartBuildInstall(
		InstalledManifest, RemoteManifest, InstallDir, 
		FBuildPatchBoolManifestDelegate::CreateStatic(&OnInstallComplete, this, OnSucceeded, OnFailed));
}
...
BuildPatchServices::EBuildPatchState UMobilePendingContent::GetInstallerState()
{
	if (Installer.IsValid())
	{
...
static void OnRemoteManifestReady(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bSucceeded, UMobilePendingContent* MobilePendingContent, FOnRequestContentSucceeded OnSucceeded, FOnRequestContentFailed OnFailed)
{
	...
	ON_SCOPE_EXIT
	{
		if (ErrorCode != ERequestContentError::NoError)
		{
			...
			UE_LOG(LogMobilePatchingUtils, Log, TEXT("ErrorText: %s. Code %d"), *ErrorText.ToString(), (int32)ErrorCode);
			OnFailed.ExecuteIfBound(ErrorText, (int32)ErrorCode);
		}
	};
	...
	{
		UE_LOG(LogMobilePatchingUtils, Log, TEXT("Failed to download manifest. ResponseCode = %d, ResponseMsg = %s"), 
...
static IBuildManifestPtr GetInstalledManifest(const FString& InstallDirectory)
{
	...
	UE_LOG(LogMobilePatchingUtils, Log, TEXT("FullInstallDir: %s"), *FullInstallDir);
	...
	FString LatestManifestName;

	// There could be multiple manifests because of an unfortunate timing of application termination. It is possible to infer order from lexicographical order
	UE_LOG(LogMobilePatchingUtils, Log, TEXT("GetInstalledManifest: Number of manifests: %d"), InstalledManifestNames.Num());
	for (int i = 0; i < InstalledManifestNames.Num(); i++)
	{
		const FString& PossibleManifestName = InstalledManifestNames[i];// should we warn if there more than one manifest?
		UE_LOG(LogMobilePatchingUtils, Log, TEXT("GetInstalledManifest: considering: %s"), *PossibleManifestName);

		IBuildPatchServicesModule* BuildPatchServices = GetBuildPatchServices();
		IBuildManifestPtr PossibleManifest = BuildPatchServices->LoadManifestFromFile(FullInstallDir / PossibleManifestName);

		if (PossibleManifest.IsValid())
		{
			if (InstalledManifest.IsValid())
			{
				if (LatestManifestName < PossibleManifestName)
				{
					InstalledManifest = PossibleManifest;
					LatestManifestName = PossibleManifestName;
				}
			}
			else
			{
	...
	UE_LOG(LogMobilePatchingUtils, Log, TEXT("EXIT GetInstalledManifest with %s"), *LatestManifestName);
...
void UMobilePatchingLibrary::RequestContent(const FString& RemoteManifestURL, const FString& CloudURL, const FString& InstallDirectory, FOnRequestContentSucceeded OnSucceeded, FOnRequestContentFailed OnFailed)
{
	...
	UE_LOG(LogMobilePatchingUtils, Log, TEXT("RemoteManifestURL: %s."), *RemoteManifestURL);
...
FString UMobilePatchingLibrary::GetOnlineAccountID(APlayerController* PlayerController)
{

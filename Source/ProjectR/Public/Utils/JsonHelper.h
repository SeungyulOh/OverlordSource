...
class PROJECTR_API FJsonHelper
{
public:

	// =============================================
	template<typename StructType>
	static bool TArrayUStructToJsonObjectString(const UStruct* StructDefinition, TArray<StructType>& StructArray, FString& OutJsonString, int64 CheckFlags, int64 SkipFlags, int32 Indent = 0)
	{
		FString strOutput = TEXT("[\n");
		for (int32 i = 0; i < StructArray.Num(); ++i)
		{
			FString strEach;
			if (FJsonHelper::UStructToJsonObjectString(StructDefinition, &StructArray[i], strEach, CheckFlags, SkipFlags, Indent))
			{
				strOutput += strEach;
				if (i == StructArray.Num() - 1)
				{
					strOutput += TEXT("\n]");
				}
				else
...
public: // JSON -> UStruct
	...
	*/
	...
	static bool JsonObjectStringToUStruct(const FString& JsonString, OutStructType* OutStruct, int64 CheckFlags, int64 SkipFlags)
	{
		...
		if (!FJsonHelper::JsonObjectToUStruct(JsonObject.ToSharedRef(), OutStruct, CheckFlags, SkipFlags))
		{
	...
	*/
	...
	static bool JsonArrayToUStruct(const TArray<TSharedPtr<FJsonValue>>& JsonArray, TArray<OutStructType>* OutStructArray, int64 CheckFlags, int64 SkipFlags)
	{
		...
		for (int32 i = 0; i < JsonArray.Num(); ++i)
		{
			...
			if (!FJsonHelper::JsonObjectToUStruct(Value->AsObject().ToSharedRef(), OutStructType::StaticStruct(), &(*OutStructArray)[i], CheckFlags, SkipFlags))
			{

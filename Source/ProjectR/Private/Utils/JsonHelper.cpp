...
FString FJsonHelper::StandardizeCase(const FString &StringIn)
{
...
namespace
{
	...
	TSharedPtr<FJsonValue> ConvertScalarUPropertyToJsonValue(UProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, const FJsonHelper::CustomExportCallback* ExportCb)
	{
		if (UNumericProperty *NumericProperty = Cast<UNumericProperty>(Property))
		{
			...
			if (EnumDef != NULL)
			{
				...
				FString StringValue = EnumDef->GetNameStringByIndex(EnumDef->GetIndexByValue(NumericProperty->GetSignedIntPropertyValue(Value)));
		...
		else if (UArrayProperty *ArrayProperty = Cast<UArrayProperty>(Property))
		{
			...
			{
				TSharedPtr<FJsonValue> Elem = FJsonHelper::UPropertyToJsonValue(ArrayProperty->Inner, Helper.GetRawPtr(i), CheckFlags & (~CPF_ParmFlags), SkipFlags, ExportCb);
				if (Elem.IsValid())
				{
					// add to the array
					Out.Push(Elem);
		...
		else if (UStructProperty *StructProperty = Cast<UStructProperty>(Property))
		{
			...
			if (FJsonHelper::UStructToJsonObject(StructProperty->Struct, Value, Out, CheckFlags & (~CPF_ParmFlags), SkipFlags, ExportCb))
			{
...
TSharedPtr<FJsonValue> FJsonHelper::UPropertyToJsonValue(UProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
	...
	Array.Reserve(Property->ArrayDim);
	for (int32 Index = 0; Index != Property->ArrayDim; ++Index)
	{
		Array.Emplace(ConvertScalarUPropertyToJsonValue(Property, (char*)Value + Index * Property->ElementSize, CheckFlags, SkipFlags, ExportCb));
	}
	return MakeShareable(new FJsonValueArray(Array));
}

bool FJsonHelper::UStructToJsonObject(const UStruct* StructDefinition, const void* Struct, TSharedRef<FJsonObject> OutJsonObject, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
	return UStructToJsonAttributes(StructDefinition, Struct, OutJsonObject->Values, CheckFlags, SkipFlags, ExportCb);
}

bool FJsonHelper::UStructToJsonAttributes(const UStruct* StructDefinition, const void* Struct, TMap< FString, TSharedPtr<FJsonValue> >& OutJsonAttributes, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
	...
	for (TFieldIterator<UProperty> It(StructDefinition); It; ++It)
	{
		...
		if (!JsonValue.IsValid())
		{
			...
			UE_LOG(LogJson, Log, TEXT("UStructToJsonObject - Unhandled property type '%s': %s"), *PropClass->GetName(), *Property->GetPathName());
			return false;
		}
...
bool FJsonHelper::UStructToJsonObjectString(const UStruct* StructDefinition, const void* Struct, FString& OutJsonString, int64 CheckFlags, int64 SkipFlags, int32 Indent, const CustomExportCallback* ExportCb)
{
	...
	bool ConvertScalarJsonValueToUProperty(TSharedPtr<FJsonValue> JsonValue, UProperty* Property, void* OutValue, int64 CheckFlags, int64 SkipFlags)
	{
		if (UNumericProperty *NumericProperty = Cast<UNumericProperty>(Property))
		{
			if (NumericProperty->IsEnum() && JsonValue->Type == EJson::String)
			{
				...
				{
					UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Unable import enum %s from string value %s for property %s"), *EnumProperty->CppType, *StrValue, *Property->GetNameCPP());
					return false;
				}
			...
			{
				UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Unable to set numeric property type %s for property %s"), *Property->GetClass()->GetName(), *Property->GetNameCPP());
				return false;
			}
		}
		else if (UBoolProperty *BoolProperty = Cast<UBoolProperty>(Property))
		...
		else if (UArrayProperty *ArrayProperty = Cast<UArrayProperty>(Property))
		{
			if (JsonValue->Type == EJson::Array)
			{
				...
				for (int32 i = 0; i < ArrLen; ++i)
				{
					...
					{
						if (!FJsonHelper::JsonValueToUProperty(ArrayValueItem, ArrayProperty->Inner, Helper.GetRawPtr(i), CheckFlags & (~CPF_ParmFlags), SkipFlags))
						{
							UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Unable to deserialize array element [%d] for property %s"), i, *Property->GetNameCPP());
							return false;
						}
					}
				}
			...
			{
				UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Attempted to import TArray from non-array JSON key for property %s"), *Property->GetNameCPP());
				return false;
			}
		}
		else if (UTextProperty *TextProperty = Cast<UTextProperty>(Property))
		{
			...
			else if (JsonValue->Type == EJson::Object)
			{
				...
				{
					UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Attempted to import FText from JSON object with invalid keys for property %s"), *Property->GetNameCPP());
					return false;
				}
			...
			{
				UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Attempted to import FText from JSON that was neither string nor object for property %s"), *Property->GetNameCPP());
				return false;
			}
		}
		else if (UStructProperty *StructProperty = Cast<UStructProperty>(Property))
		{
			...
			if (JsonValue->Type == EJson::Object)
			{
				TSharedPtr<FJsonObject> Obj = JsonValue->AsObject();
				check(Obj.IsValid()); // should not fail if Type == EJson::Object
				if (!FJsonHelper::JsonObjectToUStruct(Obj.ToSharedRef(), StructProperty->Struct, OutValue, CheckFlags & (~CPF_ParmFlags), SkipFlags))
				{
					UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - FJsonHelper::JsonObjectToUStruct failed for property %s"), *Property->GetNameCPP());
					return false;
				}
			}
			else if (JsonValue->Type == EJson::String && StructProperty->Struct->GetFName() == NAME_DateTime)
			{
				...
				{
					UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Unable to import FDateTime from Iso8601 String for property %s"), *Property->GetNameCPP());
					return false;
				}
			}
			else
			{
				UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Attempted to import UStruct from non-object JSON key for property %s"), *Property->GetNameCPP());
				return false;
			}
		}
		else
		{
			...
			{
				UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Unable import property type %s from string value for property %s"), *Property->GetClass()->GetName(), *Property->GetNameCPP());
				return false;
			}
		}
...
bool FJsonHelper::JsonValueToUProperty(TSharedPtr<FJsonValue> JsonValue, UProperty* Property, void* OutValue, int64 CheckFlags, int64 SkipFlags)
{
	if (!JsonValue.IsValid())
	{
		UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Invalid value JSON key"));
		return false;
	}
	...
	if (!bJsonArray)
	{
		if (bArrayProperty)
		{
			UE_LOG(LogJson, Log, TEXT("JsonValueToUProperty - Attempted to import TArray from non-array JSON key"));
			return false;
		}
	...
	int32 ItemsToRead = FMath::Clamp(ArrayValue.Num(), 0, Property->ArrayDim);
...
bool FJsonHelper::JsonObjectToUStruct(const TSharedRef<FJsonObject>& JsonObject, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags, int64 SkipFlags)
{
	return JsonAttributesToUStruct(JsonObject->Values, StructDefinition, OutStruct, CheckFlags, SkipFlags);
}

bool FJsonHelper::JsonAttributesToUStruct(const TMap< FString, TSharedPtr<FJsonValue> >& JsonAttributes, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags, int64 SkipFlags)
{
	if (StructDefinition == FJsonObjectWrapper::StaticStruct())
	{
		// Just copy it into the object
	...
	for (TFieldIterator<UProperty> PropIt(StructDefinition); PropIt; ++PropIt)
	{
		...
		{
			UE_LOG(LogJson, Log, TEXT("JsonObjectToUStruct - Unable to parse %s.%s from JSON"), *StructDefinition->GetName(), *PropertyName);
			return false;
		}
	}
